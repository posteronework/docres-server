...
def mask_base_dewarper(image,mask):
    ...
    source_points = torch.from_numpy(source_points).float().to(target_points.device)
    ...
    # model moved to device externally
    grid = model(source_points).view(-1,256,256,2).permute(0,3,1,2)
    grid = F.interpolate(grid,(h,w),mode='bilinear').permute(0,2,3,1)
    dewarped = MBD_utils.torch2cvimg(F.grid_sample(MBD_utils.cvimg2torch(image).to(source_points.device),grid))[0]
